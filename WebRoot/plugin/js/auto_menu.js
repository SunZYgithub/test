		...
		$.ajax({
			...
			success: function (data) {
				...
				for(i=0;i<data.length;i++){					
					if(data[i].PREV_CODE==null||data[i].PREV_CODE==''){
						...
						if(data[i].FUNC==null||data[i].FUNC==''){
							...
							/*$('.'+data[i].PREV_CODE+' ul').each(function(me,num) {
								var me = $(this);
								num = parseFloat(me.text());
								alert(num)
								$(new Array(num)).each(function(j, ele) {
									me.append("<li class=\"treeview "+data[i].CODE+"\"><a href=\"#\"><i class=\""+data[i].ICON+"\"></i><span>"+data[i].NAME+"</span><i class=\"fa fa-angle-left pull-right\"></i></a><ul class=\"treeview-menu\"></ul></li>");
								})
							})*/
							...
							//$('.'+data[i].PREV_CODE+' ul:even').append("<li><a href=\"#\" data-href=\""+data[i].HREF+"\" data-funcid=\""+data[i].FUNC_ID+"\"><i class=\""+data[i].ICON+"\"></i>"+data[i].FUNC+"</a></li>");
							...
							$('.'+data[i].PREV_CODE+' ul').each(function(me,num) {
								...
								num = parseFloat(me.text());
								$(new Array(num)).each(function(j, ele) {
									me.append("<li><a href=\"#\" data-href=\""+data[i].HREF+"\" data-funcid=\""+data[i].FUNC_ID+"\"><i class=\""+data[i].ICON+"\"></i>"+data[i].FUNC+"</a></li>");
								})
							})
							*/
						}else{
				...
				$('a[data-href]').on('click', function(e) {
					...
					for(i=json.length;i>0;i--){
						$('ol.breadcrumb').append('<li data-hrefflag="N">'+json[i-1].innerText+'</li>');				
					}
					/*for(i=0;i<json.length;i++){
					    $('ol.breadcrumb').prepend('<li data-hrefflag="N">'+json[i].innerText+'</li>');
					    //alert(json[i].innerText);
					}*/
